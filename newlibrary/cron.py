...
def main(**params):
    ...
    if do_install:
        ...
        if len(old_job) > 0:
            ...
            return module.exit_json(changed=changed,cron_file=cron_file,state=state)
    ...
    if not backup:
        os.unlink(backupfile)
        return module.exit_json(changed=changed,jobs=jobnames)
    else:
        return module.exit_json(changed=changed,jobs=jobnames,backup=backupfile)
