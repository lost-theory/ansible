...
def main(**params):
    ...
    if creates is not None:
        ...
        if os.path.exists(creates):
            return module.exit_json(stdout="skipped, since %s exists" % creates, skipped=True, changed=False, stderr=False, rc=0)
    ...
    if removes is not None:
        ...
        if not os.path.exists(removes):
            return module.exit_json(stdout="skipped, since %s does not exist" % removes, skipped=True, changed=False, stderr=False, rc=0)
    ...
    if resp['status'] != status_code:
        ...
        return module.exit_json(changed=True, content=content, **uresp)  
    else:
        return module.exit_json(changed=True, **uresp)
