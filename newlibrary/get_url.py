...
def url_get(module, url, dest, use_proxy):
    ...
    if info['status'] == 304:
        return module.exit_json(url=url, dest=dest, changed=False, msg=info.get('msg', ''))
...
def main(**params):
    ...
    if not force:
        if os.path.exists(dest):
            return module.exit_json(msg="file already exists", dest=dest, url=url, changed=False)
    ...
    # Mission complete
    return module.exit_json(url=url, dest=dest, src=tmpsrc, md5sum=md5sum_src,
