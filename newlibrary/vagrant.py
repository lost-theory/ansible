    ...
    try:
        if state != None:
            ...
            if state == 'present':
                ...
                return module.exit_json(changed = changd, instances = insts)
                 
            if state == 'absent':
                changd = vgw.halt(vm_name)    
                return module.exit_json(changed = changd, status = vgw.status(vm_name))
...
#                module.fail_json(msg = "Error: you must specify a vm_name when calling status." )
                ...
                return module.exit_json(changed = changd, status = result)
                ...
                return module.exit_json(changed = changd, config = cnf)
                ...
                return module.exit_json(changed = changd, msg = sshmsg, SshCommand = sshcmd)
           
            elif cmd == 'halt':

                (changd, stats) = vgw.halt(vm_name)
                return module.exit_json(changed = changd, status = stats)
                ...
                changd = vgw.clear()          
                return module.exit_json(changed = changd)
    ...
    return module.exit_json(status = "success")
