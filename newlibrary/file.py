...
def main(**params):
    ...
    if params.get('diff_peek', None) is not None:
        ...
        try:
            ...
            pass
        return module.exit_json(path=path, changed=False, appears_binary=appears_binary)
            ...
            if prev_state == 'directory':
                if os.path.islink(path):
                    ...
                    try:
                        ...
                        return module.exit_json(msg="rmtree failed")
            else:
                os.unlink(path)
        except Exception, e:
            module.fail_json(path=path, msg=str(e))
        return module.exit_json(path=path, changed=True)

    if prev_state != 'absent' and prev_state != state:
        module.fail_json(path=path, msg='refusing to convert between %s and %s for %s' % (prev_state, state, src))

    if prev_state == 'absent' and state == 'absent':
        return module.exit_json(path=path, changed=False)
    ...
    if state == 'file':
        ...
        return module.exit_json(path=path, changed=changed)
        ...
        return module.exit_json(dest=path, src=src, changed=changed)
